...
void write_header(void){
    ...
    fprintf(Stream, " with some Modification by Takama M.\n");
...
inline double x_w2n(double x){
    return ((x-Xw1)/(Xw2-Xw1)*(Xv2-Xv1) + Xv1);
}

inline double y_w2n(double y){
    return ((y-Yw1)/(Yw2-Yw1)*(Yv2-Yv1) + Yv1);
}

inline double x_w2paper(double x){
    return x_w2n(x)*Xsize;
}

inline double y_w2paper(double x){
    return y_w2n(x)*Ysize;
}

inline double x_scale_w2paper(double sx){
    return sx/(Xw2-Xw1)*(Xv2-Xv1)*Xsize;
}

inline double y_scale_w2paper(double sy){
    return sy/(Yw2-Yw1)*(Yv2-Yv1)*Ysize;
}

inline double x_n2w(double xn){
    return ((xn*Xsize-Xv1)/(Xv2-Xv1)*(Xw2-Xw1) + Xw1)/Xsize;
}

inline double y_n2w(double yn){
...
void fillseries1(double xx[], double yy[], int n){
    int i;
    plot(xx[0],yy[0],MOVETO);
    for(i=1; i<n; i++){
        plot(xx[i],yy[i],LINETO);
...
{
    newpath();plot(x1,y1,MOVETO);plot(x2,y1,LINETO);plot(x2,y2,LINETO);plot(x1,y2,LINETO);
    closepath();
    stroke();
    return;
}
...
void grid_with_text(double x1, double y1, double x2, double y2, int nrow, int ncol,
                    char *str)
{
    double x, y;
    double dx = (x2-x1)/ncol, dy = (y2-y1)/nrow;
...
void arrow1(double x1, double y1, double x2, double y2, double d)
{
    ...
    plot(x2, y2, MOVETO); plot(x3, y3, LINETO); plot(x2, y2, MOVETO); plot(x4, y4, LINETO); 
    stroke(); 
    return; 
}
...
// arrow, arrow1の改善版
// d: 矢印の大きさのパラメータ(cm単位)
void arrow01(double x1, double y1, double x2, double y2, double d, double g, double w)
{
    linewidth(w); setgray(g); 
...
void resist(double x1, double y1, double x2, double y2, double d, double w)
{
    ...
    ex=-d*dy; ey=d*dx; 
    plot(x1, y1, MOVETO); xx=x1+dx+ex; yy=y1+dy+ey; plot(xx, yy, LINETO); 
    for(i=1;i<=5; i++){
...
void battery(double x1, double y1, double x2, double y2, double d, double w)
{
    ...
    ex=-dy; ey=dx; 
    plot(x1, y1, MOVETO); xx=x1+dx; yy=y1+dy; plot(xx, yy, LINETO); stroke();
    linewidth(2.0*w); 
    xx=x1+dx+3.0*ex; yy=y1+dy+3.0*ey; 
    plot(xx, yy, MOVETO); xx=x1+dx-3.0*ex; yy=y1+dy-3.0*ey; plot(xx, yy, LINETO); stroke(); 
    xx=x1+2.0*dx+ex; yy=y1+2.0*dy+ey; 
    plot(xx, yy, MOVETO); xx=x1+2.0*dx-ex; yy=y1+2.0*dy-ey; plot(xx, yy, LINETO); stroke(); 
    linewidth(w);
    plot(x2, y2, MOVETO); xx=x2-dx; yy=y2-dy; plot(xx, yy, LINETO); stroke(); 
...
void offset_wrt_line(lbAlign a, int ftsize, int n,double *xs, double *ys){
    switch(a){
        case LEFT : 
            *xs = -(n+1)*ftsize/2.0; *ys = - 1.0/3.0*ftsize;
            break;
        case BELOW : 
